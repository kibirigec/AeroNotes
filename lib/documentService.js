...
/**
 * Fetch all document-type files for the current user
 */
...
export const createDocument = async (formData) => {
  ...
  const uniqueFileName = `${user.id}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9._-]/g, '')}`;
...
export const updateDocument = async (id, updates) => {
  ...
  // Ensure user_id in updates is not tampered with, or rely on RLS
  const validUpdates = { ...updates };
  delete validUpdates.user_id; // Prevent user_id spoofing if passed in updates
