...
export function AuthProvider({ children }) {
  ...
  const autoSignoutState = useAutoSignout({
    ...
    desktopTimeoutMs: (autoSignoutSettings?.isMobile ? null : autoSignoutSettings?.timeoutMinutes) * 60 * 1000,
    mobileTimeoutMs: (autoSignoutSettings?.isMobile ? autoSignoutSettings?.timeoutMinutes : null) * 60 * 1000,
