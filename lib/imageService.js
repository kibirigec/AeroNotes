...
export const fetchImageMetadata = async () => {
  ...
  return data.map(img => {
    // Add cache-busting for images uploaded in the last 5 minutes
    const uploadTime = new Date(img.created_at);
    const now = new Date();
    const isRecent = (now - uploadTime) < 5 * 60 * 1000; // 5 minutes
    
    let url = supabase.storage.from(img.bucket_id).getPublicUrl(img.file_path).data.publicUrl;
    
    // Add cache-busting parameter for recent uploads
    if (isRecent) {
      const cacheBuster = `?cb=${uploadTime.getTime()}`;
      url += cacheBuster;
    }
...
export const uploadImage = async (file, alt, autoDelete = false, expiryDays = DEFAULT_IMAGE_EXPIRY_DAYS) => {
  ...
  return {
    ...dbData,
    url: supabase.storage.from(dbData.bucket_id).getPublicUrl(dbData.file_path).data.publicUrl + `?cb=${Date.now()}`,
    isRecent: true
